...
/*   Updated: 2019/01/12 20:35:42 by jlucas-l         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mandelbrot(t_var *var, int x, int y)
{
	...
	var->cx.re = (((double)x / W_WIDTH) * (var->f.x_max - var->f.x_min)) *
		var->opt.scale + var->f.x_min - var->f.offx;
	var->cx.im = (((double)y / W_HEIGHT) * (var->f.y_max - var->f.y_min)) *
		var->opt.scale + var->f.y_min - var->f.offy;
	var->cx.zre = 0;
	var->cx.zim = 0;
...
void	burningship(t_var *var, int x, int y)
{
	...
	set_pixel(var, x, y, n == var->opt.iter ? 0 : get_color(n % 7));
}
